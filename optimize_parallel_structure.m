function objective_value = optimize_parallel_structure(params, configuration, fibertype, freq, airProperties, coefficients, area_ratios, target_freq)
    ...
    if configuration == 1  % Fiber + Air
        ...
        [~, ~, alpha, Re, Im] = parallel_acoustic_structure(configuration, fibertype, [fiber_thickness, fiber_porosity, air_thickness], freq, airProperties, coefficients, area_ratios);
    ...
    elseif configuration == 2  % Fiber + Fiber
        ...
        % Call the multi_layer_acoustics function
        [~, ~, alpha, Re, Im] = parallel_acoustic_structure(configuration, fibertype, [fiber1_thickness, fiber1_porosity, fiber2_thickness, fiber2_porosity], freq, airProperties, coefficients, area_ratios);
    ...
    elseif configuration == 3  % MPP + Fiber
        ...
        mpp_air_thickness = params(6); % Air layer thickness (mm)
         
        % Call the multi_layer_acoustics function
        [~, ~, alpha, Re, Im] = parallel_acoustic_structure(configuration, fibertype, [fiber_thickness, fiber_porosity, mpp_perforation_rate, mpp_micropore_r, mpp_thickness, mpp_air_thickness], freq, airProperties, coefficients, area_ratios);
              
    else
        error('Invalid configuration');
    end
    
    % Find first perfect absorption frequency
    ...
    freq_penalty = abs(first_perfect_absorption_freq - target_freq);
