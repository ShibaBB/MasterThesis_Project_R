"""Plan or execute matched full-range Global SR Sobol feature experiments."""
...
from global_sobol_feature_policy import SUPPORTED_BRANCHES
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run matched all-feature and Sobol-subset full-range Global SR experiments."
    )
    parser.add_argument("--dataset-run", default="run1")
    parser.add_argument("--targets", nargs="+", choices=("re", "im"), default=["re", "im"])
    parser.add_argument(
        "--branches", nargs="+", choices=SUPPORTED_BRANCHES,
    ...
    parser.add_argument("--run-label", default="sobol_phase1")
    parser.add_argument(
...
def main() -> None:
    ...
    records = [
        ...
        for target in args.targets for branch in args.branches
    ]
    manifest = {
        "schema_version": 1,
        "purpose": "matched teacher-Sobol-guided full-range Global SR feature experiment",
        "created_at": datetime.now().astimezone().isoformat(),
        "execute": args.execute,
        "frequency_domains": [{"lower_hz": 100.0, "upper_hz": 4950.0}],
        "selection_contract": (
            "Each target chooses one whole-range feature branch using validation RMSE only; "
            "test metrics are reported only after that choice and no segment hybrid is allowed."
        ),
        "matched_settings": {
    ...
    if args.execute and {"all", "sobol_subset"}.issubset(args.branches):
        ...
        for target in args.targets:
            by_branch = {
                record["branch"]: record for record in records
